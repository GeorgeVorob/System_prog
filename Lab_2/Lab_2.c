...
int main(int argc, char *argv[])
{
    ...
    if(fstat(filedesc,&filestat)<0) //заполнение структуры информацией
    {
        perror("stat error");
        return 1;
    }

    char *mmapptr = mmap(NULL,filestat.st_size,PROT_READ,MAP_SHARED,filedesc,0); //загрузка файла в память
    ...
    for (int i=0;i<=filestat.st_size / sizeof(char);i++) //Считывание файла, счёт кол-ва и длины строк
    {
        strCount++;
        if (mmapptr[i] == '\n')
        {
